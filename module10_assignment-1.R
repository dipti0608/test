...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(inputId = "subset_income",label = "select income", choices = c("<=50K", "50K")),
      # Interactive piece 2: inputId = "set_yaxis"
      selectInput(inputId = "set_yaxis", label = "select Y-axis", choices = c(Hours_per_week = "hours_per_week",capital_loss = "capital_loss")),
